...
include 'connexion.php';
	...
	$req=$db->prepare("SELECT CONCAT(locataire.prenom,' ',locataire.nom), CONCAT(day(location.date_debut),' ', monthname(location.date_debut),' ', year(location.date_debut)), location.caution, location.commission, location.prix_location, type_logement.type_logement, logement.designation, logement.adresse 
FROM `location`, locataire, logement, type_logement 
WHERE type_logement.id=logement.id_type AND location.id_logement=logement.id AND location.id_locataire=locataire.id AND location.id=?");
...
<html>
	...
	<body style="background-image: url('<?=$image ?>i_recu_bailleur.jpg'); font: 12pt 'times new roman';" >
		...
		<a href="i_contrat_location.php?id=<?=$_GET['id'] ?>" class="btn" >Contrat de location</a>
		<div class="container  white" style="padding:  10px">
			<div class="row center" >
				<img class="col s12" src="../css/images/entete_immo.jpg" >
			...
			<div class="row">
				<h6 class="col s12 center">
					Arrêtée à la présente somme de  <b><?= number_format($total,0,'.',' ') ?> Fcfa</b> Fcfa.....<b><i><?=$formatter->format($total); ?> Fcfa</i></b>....
				</h6>
			</div>
			<div class="row">
