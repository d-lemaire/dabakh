...
$reponse=$db->prepare("SELECT patient.id_patient, CONCAT(patient.prenom,' ', patient.nom), CONCAT(day(d_regularisation.date_obs),' ',monthname(d_regularisation.date_obs),' ',year(d_regularisation.date_obs)), CONCAT(day(d_regularisation.date_reg),' ',monthname(d_regularisation.date_reg),' ',year(d_regularisation.date_reg)), d_regularisation.montant, patient.sexe, patient.situation_matrimoniale, patient.num_dossier, consultation_domicile.id_consultation, patient.annee_inscription
FROM `d_regularisation`, patient, consultation_domicile
WHERE  patient.id_patient=consultation_domicile.id_patient AND d_regularisation.id_cons_domicile=consultation_domicile.id_consultation AND d_regularisation.id_reg=?");
...
<html>
	...
	<body style="background-image: url('<?=$image ?>dossier_patient.png');">
		...
		<div class="container white" style="border-radius: 15px;">
			...
			<h6 class="center"><b  style="font: 22pt 'times new roman';">Demande de régularisation soins à domicile <br><b>N° <?= str_pad($id_cons_domicile, 4, "0", STR_PAD_LEFT) ?></b></b></h6>
			<div class="row" style="padding: 5px; ">
				...
				<br>
				<br>
				Ayant subit à la date du <b><?=$date_obs?></b> les soins à domicile suivants :<br>
				<?php
						$total=0;
						...
						$req_consultation->execute(array($id_cons_domicile));
