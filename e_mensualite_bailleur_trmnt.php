...
else
{
	...
	$req->execute(array($id_mensualite_bailleur, $montant, $mois, $annee, $date_versement, $non_recouvrer, $commission, $_GET['id'], $id_user, $type_paiement)) or die(print_r($req->errorInfo()));
	$nbr=$req->rowCount();
	...
	$donnees=$req->fetch();
	$motif="Payement ".$mois." pour le bailleur ".$donnees['0'];
	$req->closeCursor();
	...
	if ($mois_a_payer=="tous") 
	{
		$req=$db->prepare("SELECT  mensualite.id
			...
			WHERE logement.id_type=type_logement.id AND mensualite.id_location=location.id AND location.id_logement=logement.id AND logement.id_bailleur=bailleur.id AND mensualite.mois<>? AND bailleur.id=? AND mensualite.id_mensualite_bailleur=0"); 
		$req->execute(array($mois, $_GET['id']));
