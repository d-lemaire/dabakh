...
if ($search=="") 
{
	...
	$reponse=$db->prepare("SELECT id, CONCAT(DATE_FORMAT(date_operation, '%d'), '/', DATE_FORMAT(date_operation, '%m'),'/', DATE_FORMAT(date_operation, '%Y')), motif, type, montant, id_mensualite,section, id_mensualite_bailleur, id_depense_bailleur, id_user, id_depense_locataire,pj
	FROM `caisse_immo`
	...
	if ($nbr>0) 
	{
		...
		while ($donnees= $reponse->fetch())
		{
			...
			if (isset($id_mensualite)) 
				{
					echo "<td class='center'><a href='i_mensualite.php?id=".$id_mensualite."'>N° ".$id_mensualite."</a></td>";	
				}
			elseif (isset($id_depense_bailleur)) 
				{
					echo "<td class='center'>N° ".$id_depense_bailleur."</td>";						
				}
			elseif (isset($id_mensualite_bailleur)) 
				{
					echo "<td class='center'><a href='i_mensualite.php?id=".$id_mensualite_bailleur."'>N° ".$id_mensualite_bailleur."</a></td>";	
				}
			else
				{
					if ($section<>"solde") 
