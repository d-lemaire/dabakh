...
if (isset($_GET['id_patient'])) 
{
	...
	$req=$db->prepare('INSERT INTO d_consultation (examen, renseignements, id_patient, date_prescription) VALUES (:examen,:renseignements,:id_patient,:date_prescription)');
	$req->execute(array('examen'=>$examen,'renseignements'=>$renseignements, 'id_patient'=>$id_patient, 'date_prescription'=>$date_prescription));
	$nbr=$req->rowCount();
...
$reponse=$db->prepare("SELECT patient.id_patient, CONCAT(patient.prenom,' ', patient.nom), d_consultation.examen, d_consultation.renseignements, patient.sexe, patient.situation_matrimoniale, CONCAT(CONCAT(day(patient.date_naissance),' ', monthname(patient.date_naissance),' ', year(patient.date_naissance)), ' à ',patient.lieu_naissance)
			...
			<tbody>
				<div class="container white" style="border-radius: 15px;">
					<div class="row" style="padding: 5px; ">
						<h4 class="col s12 center"><u>Demande de consultation spécialisée</u></h4>
						<br>
						<h5 class="col s12 ">
							Prénom et Nom : 
