...
{
	$reponse=$db->prepare("SELECT * FROM `bailleur` WHERE CONCAT (prenom,' ',' ',nom) like CONCAT('%', ?, '%') AND etat='activer' ORDER BY annee_inscription, num_dossier ASC");
...
while ($donnees= $reponse->fetch())
{
	...
	echo "<tr>";
		...
		{
		echo "<a class='red btn' href='supprimer.php?id_bailleur=".$id."' onclick='return(confirm(\"Voulez-vous supprimer ce bailleur ainsi que tous les logements de ce dernier ?\"))'>Supprimer </a>";
		}
		echo "</td>";
		...
		echo "<td>Total : <b>".$total."</b>; <br>Occupé(s) : <b>".$nbr_occupe."</b> ; <br>Libre(s) : <b>";
			...
			echo "</b>";
			echo "<br><a class='btn brown' href='list_logement.php?id=$id'>Détails</a>";
		echo " </td>";
		echo "<td> <a class='btn cyan' href='journal_caisse_bailleur.php?id=$id'>Journal de caisse</a></td>";
		echo "<td> <a class='btn cyan' href='e_mensualite_bailleur.php?id=$id'>Mensualité</a></td>";
	echo "</tr>";
