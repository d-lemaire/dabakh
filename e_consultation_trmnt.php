...
if ($_SESSION['fonction']=="infirmier") 
{
	...
	$req=$db->prepare('UPDATE consultation SET date_consultation=?, poids=?, pouls=?, tension=?, temperature=?, etat=?, glycemie=?, tdr=?,  allergie=?,  plaintes=?, id_user_i=? WHERE id_consultation=?');
	$nbr=$req->execute(array($date_consultation, $poids, $pouls, $tension, $temperature, $etat, $glycemie, $tdr,  $allergie,  $plaintes, $id_user, $_GET['id']))  or die(print_r($req->errorInfo()));
	$req->closeCursor();
	if ($nbr>0) {
	?>
